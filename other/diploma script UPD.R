...
data$lagTransfer <- stats::lag(data$Transfer) #Как делать лаги.
data$deltaTransfer <-data$Transfer - data$lagTransfer

data$lagRev <- stats::lag(data$Rev) #Как делать лаги.
data$deltaRev<-data$Rev - data$lagRev
...
data$depend <-data$Transfer/data$Rev
data$depend1 <-data$grat_sub/data$Rev

data1$logrev <-log(data1$Rev)
data1$laglogrev <- stats::lag(data1$logrev) #Как делать лаги.
data1$deltalogrev <-data1$logrev - data1$laglogrev


data$lagTransferS <- stats::lag(data$grat_sub) #Как делать лаги.
data$deltagratS <-data$grat_sub - data$lagTransferS

data$lagbal <- stats::lag(data$bal_dot) #Как делать лаги.
data$deltabal <-data$bal_dot - data$lagbal

data$lagTransfer <- stats::lag(data$Transfer) #Как делать лаги.
data$deltagrat <-data$Transfer - data$lagTransfer
...
model1 <- feols (deltagrat ~ deltaRev + I(deltaRev*year20) + I(deltaRev*year21) + log(lagGRP) + log(populat)
...
model2 <- pgmm (deltaTransfer ~ lag(deltaTransfer) + deltaRev + I(deltaRev*year20) + I(deltaRev*year21) + log(lagGRP) + 
                  log(populat)
                | lag(deltaTransfer, 2:5)
                | lag(deltaRev, 1:5),
...
model3 <- pspatfit(formula = deltaTransfer ~ deltaRev + add + add1 + 
...
model4 <- spml (deltaTransfer ~ deltaRev + I(deltaRev*year20) + I(deltaRev*year21)+
...
model1_1 <- feols (deltaTransferS ~ deltaRev + I(deltaRev*year20) + I(deltaRev*year21) + log(lagGRP) + log(populat)
                   |id_N + year, panel.id = ~id_N + year, data = data)
...
model2_1 <- pgmm (deltaTransferS ~ lag(deltaTransferS) + deltaRev + I(deltaRev*year20) + I(deltaRev*year21) + log(lagGRP) + 
                    log(populat)
                  | lag(deltaTransferS, 2:5)
                  | lag(deltaRev, 1:5),
                  model = "twosteps", data = data, index = c("id_N", "year"), transformation = 'ld', fsm = 'full', width = 3)
...
model3_1 <- pspatfit(formula = deltaTransferS ~ deltaRev + add + add1 + 
                       +log(lagGRP) + log(populat), data = oop, listw = Wbin, type = "sar", 
                     zero.policy = TRUE, index = c("id", "year"))
summary(model3_1)
...
model4_1 <- spml (deltaTransferS ~ deltaRev + I(deltaRev*year20) + I(deltaRev*year21)+
                    log(lagGRP) + log(populat),
                  data = oop, zero.policy = TRUE,
                  index=c("id","year"),
                  listw = Wbin, effect = "twoways",
...
model1_2 <- feols (deltaTransferS ~ deltaRev + I(deltaRev*year20) + I(deltaRev*year21) + log(lagGRP) + log(populat)
...
model2_2 <- pgmm (deltaTransferS ~ lag(deltaTransferS) + deltaRev + I(deltaRev*year20) + I(deltaRev*year21) + log(lagGRP) + 
                    log(populat)
                  | lag(deltaTransferS, 2:5)
                  | lag(deltaRev, 1:5),
...
model4_2 <- spml (deltaTransferS ~ deltaRev + I(deltaRev*year20) + I(deltaRev*year21)+
                    log(lagGRP) + log(populat),
...
ols <- lm(deltaTransfer ~  0+ deltaRev + I(deltaRev*year20) + I(deltaRev*year21)+ log(lagGRP) + log(populat), data = data)
summary(ols)
...
modelka1 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20 + republic, data = covid20)
summary(modelka1, robust=TRUE) 

modelka2 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_media20 + republic, data = covid20)
summary(modelka2, robust=TRUE) 

modelka3 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20 + pol_constit20, data = covid20)
summary(modelka3, robust=TRUE) 

modelka4 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20 + pol_constit20, data = covid20)
summary(modelka4, robust=TRUE) 


modelka1_1 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20 + pol_president, data = covid20)
summary(modelka1_1, robust=TRUE) 

modelka2_1 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_media20 + pol_president, data = covid20)
summary(modelka2_1, robust=TRUE) 

modelka3_1 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20 + pol_president, data = covid20)
summary(modelka3_1, robust=TRUE) 

modelka4_1 <- lm (log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20 + pol_president, data = covid20)
summary(modelka4_1, robust=TRUE) 



modelka5 <- lm (log(I(covid21/Rev)) ~ log(lagGRP) + log(populat) + death_excess21 + republic, data = covid21)
summary(modelka5, robust=TRUE) 

modelka6 <- lm (log(I(covid21/Rev)) ~ log(lagGRP) + log(populat) + death_excess21 + pol_constit21, data = covid21)
summary(modelka6, robust=TRUE) 

modelka7 <- lm (log(I(covid21/Rev)) ~ log(lagGRP) + log(populat) + death_excess2020 + pol_constit21, data = covid21)
summary(modelka7, robust=TRUE) 

modelka8 <- lm (log(I(covid21/Rev)) ~ log(lagGRP) + log(populat) + death_excess2020 + republic, data = covid21)
summary(modelka8, robust=TRUE)



modelka5_1 <- lm (log(I(covid21/Rev)) ~ log(lagGRP) + log(populat) + death_excess21 + pol_president, data = covid21)
summary(modelka5_1, robust=TRUE) 


modelka7_1 <- lm (log(I(covid21/Rev)) ~ log(lagGRP) + log(populat) + death_excess2020 + pol_president, data = covid21)
summary(modelka7_1, robust=TRUE) 



lambdas <- seq(0.6, 10, by = 0.05) # для этих лямбд будем делать RR
m.rr <- lm.ridge(log(I(covid20/Rev)) ~ log(lagGRP) + log(populat) + death_excess20+ republic,
...
X <- model.matrix(~ 0 + log(lagGRP) + log(populat) + death_media20 + Rev +republic + pol_constit20, data = covid20)
cor(X)
XX <- t(X) %*% X
eigen <- eigen(XX)
eigen$values
